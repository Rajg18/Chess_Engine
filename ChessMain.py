...
def main():
    ...
    while running:
        ...
        if not gameOver and not humanTurn:
            AIMove = MinMax.findBestMoveMinimax(gs, validMoves, depth = 5, alpha=-float('inf'), beta=float('inf'),
                                                maximizingPlayer=(gs.whiteToMove))
            if AIMove is None:  # If no move found (in rare cases)
...
def animateMove(move, screen, board, clock):
    colors = [p.Color("forest green"), p.Color("beige")]
